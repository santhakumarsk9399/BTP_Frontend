    ...
    const fetchEditData = async () => {
      ...
      try {
        ...
        if (response?.data?.success && response?.data?.data) {
          ...
          // Get employee names from selectedEmployeeInfo
          let employeeList = [];
          if (isGroupBooking && selectedEmployeeInfo.length > 0) {
            employeeList = selectedEmployeeInfo.map(emp => emp.EmpName);
          }

          // Get main employee (first in the list)
          const mainEmployee = selectedEmployeeInfo[0] || {};

          // Determine employee type: Check if Emp_Type is 1 but the selected employee is the logged-in user
          let employeeType = "Self";
          if (travelDetails.Emp_Type === 1 || travelDetails.Emp_Type === "1") {
            // Check if selected employee ID or name matches logged-in user
            const selectedEmpIds = employeeInfo.selectedEmpid ? 
              employeeInfo.selectedEmpid.split(',').map(id => id.trim()) : [];
            const isLoggedUserSelected = 
              selectedEmpIds.includes(loggedEmpId) || 
              selectedEmployeeInfo.some(emp => 
                String(emp.Empid) === loggedEmpId || 
                String(emp.EmpId) === loggedEmpId || 
                emp.EmpName === loggedEmpName
              );
            
            // If logged-in user is not in the selected list, it's "Others"
            employeeType = isLoggedUserSelected ? "Self" : "Others";
          }
          ...
          const transformedData = {
            ...
            employeeName: mainEmployee.EmpName || employeeDetails.empName || "",
            employeeID: mainEmployee.Empid ? String(mainEmployee.Empid) : loggedEmpId,
      ...
      <Container className="travel-form-container tcreate-form py-4">
        <Row>
          ...
          <Col>
            <Button
              type="button"
              variant="secondary mb-3 back-btn"
        ...
        <Formik
          ...
          onSubmit={async (
            ...
            try {
              // Get selected employees array - only the selected ones
              let selectedempids = [];
              if (
              ...
              const currentUserName = values.employeeName || loggedUser.name;
              if (!selectedempids.some(emp => emp.empname === currentUserName)) {
                selectedempids.push({ empname: currentUserName });
              }
